...
def detect_what_is_your_name(landmarks):
    ...
    # when index finger and thumb are touching 
    return (abs(thumb_tip - index_tip) < 0.03 and
            abs(index_tip - middle_tip) < 0.03 and
            abs(middle_tip - ring_tip) > 0.05 and
            abs(ring_tip - pinky_tip) > 0.05)
